...
OUTPUT_DIR = "figs_snn_nonrandom_DMN" # <<<--- Specific output directory
...
if cm is not None:
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=np.unique(labels_full), yticklabels=np.unique(labels_full))
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title(f'Confusion Matrix (SNN: {name})')
    cm_filename = os.path.join(OUTPUT_DIR, f'confusion_matrix_snn_{name.replace("+", "_")}.png')
...
if report:
    ...
    report_filename = os.path.join(OUTPUT_DIR, f'classification_report_snn_{name.replace("+", "_")}.csv')
...
if rs_train is not None and rs_test is not None and y_train_pca is not None and y_test_pca is not None:
    if np.any(np.std(rs_train, axis=0) > 1e-6):
        ...
        plt.title(f'PCA Projection of Reservoir States (Spike Counts - SNN: {name})')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        pca_filename = os.path.join(OUTPUT_DIR, f'pca_projection_snn_{name.replace("+", "_")}.png')
