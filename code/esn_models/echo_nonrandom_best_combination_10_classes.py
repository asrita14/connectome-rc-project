...
LEAKING_RATE = 0.3
TEST_SIZE = 0.2
OUTPUT_DIR = "figs_nonrandom_DA+VIS_10cls" # Define output directory
...
try:
    ...
    # Load data once - CHANGED TO 10 CLASSES DATA
    data_full = np.load("data_2D_manifold_10_classes/1000 samples/spike_data.npy")[:, :, 0]
    labels_full = np.load("data_2D_manifold_10_classes/1000 samples/spike_labels.npy")
    print(f"Data loaded ({data_full.shape[0]} samples, {len(np.unique(labels_full))} classes) in {time.time() - start_data_load:.2f}s.")
except FileNotFoundError:
    # Updated error message
    print("Error: Data files not found. Make sure 'data_2D_manifold_10_classes/1000 samples/' directory exists and contains the .npy files.")
    exit()
...
if name in results:
    ...
    # 可视化混淆矩阵 - Adjusted figsize and labels for 10 classes
    plt.figure(figsize=(10, 8)) # Increased size
    unique_labels_sorted = sorted(np.unique(labels_full)) # Get unique labels from data
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=unique_labels_sorted, 
                yticklabels=unique_labels_sorted)
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.title(f'Confusion Matrix for {name} (10 Classes)') # Updated title
    # Updated filename and path
    cm_filename = os.path.join(OUTPUT_DIR, f'confusion_matrix_{name.replace("+", "_")}_10cls.png')
    ...
    # Updated filename and path
    report_filename = os.path.join(OUTPUT_DIR, f'classification_report_{name.replace("+", "_")}_10cls.csv')
    ...
    if rs_train_pca is not None and rs_test_pca is not None:
        ...
        try:
            ...
            plt.figure(figsize=(12, 10)) # Keep size or adjust if needed
            ...
            plt.title(f'PCA Projection of Reservoir States ({name}, 10 Classes)') # Updated title
            plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left') # Adjust legend for 10 classes
            plt.grid(True, alpha=0.3)
            plt.tight_layout(rect=[0, 0, 0.85, 1]) # Adjust layout for legend
            # Updated filename and path
            pca_filename = os.path.join(OUTPUT_DIR, f'pca_projection_{name.replace("+", "_")}_10cls.png')
