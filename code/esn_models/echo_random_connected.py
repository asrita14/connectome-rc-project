...
LEAKING_RATE = 0.3 # Leaking rate (alpha) for state update
...
print(f"\nAccuracy (from report): {class_report['accuracy']:.4f}")

# === Save results ===
output_dir = "figs_random_connected_esn"
...
# Save reservoir states if needed (optional)
# np.save(os.path.join(output_dir, "rs_train_esn.npy"), rs_train)
# np.save(os.path.join(output_dir, "rs_test_esn.npy"), rs_test)
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=sorted(set(labels)), 
